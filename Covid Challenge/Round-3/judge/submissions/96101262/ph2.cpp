...
int main()
{
    ...
    int extra = 3 * city_count * city_count - 2 * city_count * path.size();
    ...
    while (day <= max_start_day + path.size())
    {
        for (int i = 0; i < city_count; i++)
        {
            if (move_start_day[i] > day)
                continue;
            if (hosted_count[i] == city_count - 1 && extra > 0)
            {
                move_to_the_best_neighbor(ops, i, day - (max_start_day + path.size()), day);
                extra--;
            }
    ...
    if (ops.size() > 3 * city_count * city_count)
        throw "Hello";
    cout << ops.size() << endl;
    for (auto op : ops)
