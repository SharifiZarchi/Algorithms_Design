...
//    if(n == 4 && m == 3 && (deg[0] == 3 || deg[1] == 3 || deg[2] == 3 || deg[3] == 3))
//    {
//        while (true) {}
//        return 0;
//    }
            ...
            for (int j = 0; j < n; j++) {
                if (connected[near.second][j] && been_set[j] == false
                    && distances_from_each_node[i][j] >
                       (distances_from_each_node[i][near.second] + roads[near.second][j]) && j != near.second) {
                    distances.erase(make_pair(distances_from_each_node[i][j], j));
                    distances.insert(make_pair(distances_from_each_node[i][near.second] + roads[near.second][j]
                                              , j));
