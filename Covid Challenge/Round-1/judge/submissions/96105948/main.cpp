...
private:
    ...
    list<pair<pair<int, long long int>, list<int>>> neighbours(int state,long long int cost, list<int> hash, int day);
...
public:
    ...
    void visiting_util(int state, list<int> hash, int day,long long int cost);
...
void Graph::visiting() {
    ...
    list<pair<pair<int, long long int>,list<int>>> lis = neighbours(state, 0,hash,1);
...
void Graph::visiting_util(int state, list<int> hash, int day, long long int cost) {
...
//    cout<<state<<" "<<day<<" "<<visited<<" "<<cost<<endl;
...
//    cerr<<"before "<<cost<<endl;
    for (int k = 1; k < this->V+1; ++k) {
        cost+=this->verts[k];
    }
//    cerr<<"after "<<cost<<endl;

    list<pair<pair<int, long long int>, list<int>>> lis = neighbours(state, cost, hash, day + 1);
...
list<pair<pair<int, long long int>, list<int>>> Graph::neighbours(int state,long long int cost, list<int> hash, int day) {
    list<pair<pair<int, long long int>, list<int>>> neighs;
    int temp;
    temp = state;
    int last = temp % 10;
    temp = temp / 10;
    ...
    for (i = adj[last].begin(); i != adj[last].end(); ++i) {
        list<int> hash_copy;
        hash_copy = hash;
        hash_copy.push_front(100000 + (day * 100) + this->V * 10 + i->first);
...
//        cerr<<cost<<" "<< i->second<<" "<<i->second + cost<<endl;
    ...
    while (temp != 0) {
        last = temp % 10;
        temp = temp / 10;
        list<pair<pair<int, long long int>, list<int>>>::iterator j;
        list<pair<pair<int, long long int>, list<int>>> neighs_end;
        ...
        for (j = neighs.begin(); j != neighs.end(); ++j) {
            ...
            for (i = adj[last].begin(); i != adj[last].end(); ++i) {
                list<int> hash_copy;
                hash_copy = j->second;
                hash_copy.push_front(100000 + (day * 100) + count * 10 + i->first);
                neighs_end.push_front(
...
//                cerr<<" "<< j->first.second<<" "<<j->first.second + i->second<<endl;
    ...
    list<pair<pair<int, long long int>, list<int>>>::iterator j;
    list<pair<int,pair<pair<int, long long int>, list<int>>>> neighs_end;
...
//    sort(neighs_end.begin(), neighs_end.end());
    ...
    while(!neighs_end.empty()) {
        pair<int,pair<pair<int, int>, list<int>>> temmp=neighs_end.front();
        neighs_end.pop_front();
//        if(neighs.)
